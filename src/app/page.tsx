...
// ✅ NUEVA ESTRUCTURA EN MÓDULOS:
const FILTROS_MODULO_1 = [
  { key: 'M1_F1', label: 'F1' },
  { key: 'M1_F2', label: 'F2' },
  { key: 'M1_F3', label: 'F3' },
  { key: 'M1_F4', label: 'F4' },
...
const ALL_FILTROS = [...FILTROS_MODULO_1, ...FILTROS_MODULO_2];

const PURGAS_MODULO_1 = [
  { key: 'M1_S1', label: 'S1' },
  { key: 'M1_S2', label: 'S2' },
];

const PURGAS_MODULO_2 = [
  { key: 'M2_S1', label: 'S1' },
  { key: 'M2_S2', label: 'S2' },
];

const ALL_PURGAS = [...PURGAS_MODULO_1, ...PURGAS_MODULO_2];
      ...
      <div className="p-2 flex flex-col gap-2">
        ...
        <div className="bg-white border rounded shadow-sm p-2 flex flex-col gap-3">
          ...
          <div className="flex flex-col lg:flex-row gap-3 border-t border-slate-300 pt-2 items-stretch">
            ...
            {/* LAVADO DE FILTROS (8 FILTROS EN 2 MÓDULOS), PURGAS Y OBSERVACIONES */}
          <div className="flex flex-col lg:flex-row gap-3 border-t border-slate-300 pt-2 items-stretch">
            
            {/* TABLA LAVADO DE FILTROS (MÓDULO 1 Y MÓDULO 2) */}
              ...
              <table className="border-collapse border border-slate-400 text-center text-xs w-full">
                <thead>
                  ...
                  <tr className="bg-cyan-900 text-white font-bold border-b border-slate-400 text-[11px]">
                    <th className="border border-slate-400 p-0.5 w-8" rowSpan={2}>HS</th>
                    <th className="border border-slate-400 p-0.5 bg-cyan-950/80" colSpan={4}>MÓDULO 1</th>
                    <th className="border border-slate-400 p-0.5 bg-cyan-950/80" colSpan={4}>MÓDULO 2</th>
                  </tr>
                  <tr className="bg-cyan-800 text-white font-bold border-b border-slate-400 text-[11px]">
                    {ALL_FILTROS.map((f) => (
                      <th key={f.key} className="border border-slate-400 p-0.5 w-9">{f.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={`lavado-${hs}`} className={esDelTurnoActual ? 'bg-amber-50/90 font-semibold' : 'bg-slate-50/50 opacity-60'}>
                        ...
                        {ALL_FILTROS.map((f) => {
                          ...
                          return (
                            <td key={f.key} className={`border border-slate-400 p-0 h-6 ${val === 'L' ? 'bg-red-400 text-black font-black' : val === 'M' ? 'bg-emerald-100 font-bold text-emerald-950' : ''}`}>
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild disabled={!esDelTurnoActual}>
            ...
            {/* TABLA PURGAS DE SEDIMENTADORES (MÓDULO 1 Y MÓDULO 2) */}
            <div className="overflow-x-auto shrink-0">
              <table className="border-collapse border border-slate-400 text-center text-xs w-full">
                <thead>
                  ...
                  <tr className="bg-amber-900 text-white font-bold border-b border-slate-400 text-[11px]">
                    <th className="border border-slate-400 p-0.5 w-8" rowSpan={2}>HS</th>
                    <th className="border border-slate-400 p-0.5 bg-amber-950/80" colSpan={2}>MÓDULO 1</th>
                    <th className="border border-slate-400 p-0.5 bg-amber-950/80" colSpan={2}>MÓDULO 2</th>
                  </tr>
                  <tr className="bg-amber-800 text-white font-bold border-b border-slate-400 text-[11px]">
                    {ALL_PURGAS.map((s) => (
                      <th key={s.key} className="border border-slate-400 p-0.5 w-10">{s.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={`purga-${hs}`} className={esDelTurnoActual ? 'bg-amber-50/90 font-semibold' : 'bg-slate-50/50 opacity-60'}>
                        ...
                        {ALL_PURGAS.map((s) => {
                          ...
                          return (
                            <td key={s.key} className={`border border-slate-400 p-0 h-6 ${val === 'P' ? 'bg-amber-300 font-extrabold text-amber-950' : ''}`}>
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild disabled={!esDelTurnoActual}>
                                  <button className="w-full h-full text-center font-bold outline-none flex items-center justify-center disabled:cursor-not-allowed">
                                    {val || '-'}
